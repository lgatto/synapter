...
Currently it is a requirement to run the default \Rfunction{synergise} workflow
...
synapter:::.plotIonMobilitySummary(synobj2,
...
set.seed(1) # only for demonstration
m <- synapter:::.plotIonMobilitySummary(synobj2,
                                        thresholds = seq(0, 2, by=0.01),
                                        equalCharge = TRUE)
@

<<confusionmatrix, results="asis">>=
xtable(m[1:25,], row.names=FALSE)
@
