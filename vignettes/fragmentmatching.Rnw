...
features. After applying the usual \Rfunction{synergise1} workflow (see
\Rfunction{?synergise1} and \Rfunction{?Synapter} for details) a number
...
following sections then describe the new fragment matching functionality.

\section{Running \Rfunction{synergise}}\label{sec:synergise}

One has to run the \Rfunction{synergise1} workflow before fragment
matching can be applied. Please read the general \Rpackage{synapter}
vignette for the general use of \Rfunction{synergise1}.
...
<<create-synobj2, cache=FALSE, eval=TRUE, echo=-c(1:2)>>=
cat(readLines(system.file(file.path("scripts", "create_synobj2.R"),
                          package="synapterdata")), sep = "\n")
synobj2 <- synergise1(object=synobj2, outputdir=temp())
@
...
\section{Fragment matching}\label{sec:fragmentmatching}

This method, named \Rfunction{fragmentMatching}, performs the matching of
...
to be set using the \Rfunction{setFragmentMatchingPpmTolerance}
...
The \Rfunction{plotFragmentMatching} function illustrates the details of
this fragment matching procedure. If it is called without any additional
argument every matched pair (fragment vs spectrum) is plotted.
One can use the \code{key} argument to select a special value in a column
(defined by the \code{column} argument) of the \code{MatchedEMRTs}
\code{data.frame}. E.g. if one wants to select the fragment matching results
with a high number of common peaks, e.g. 28 common peaks:

\begin{figure}[!hbt]
  \begin{center}
<<plotfm22, dev ='pdf', fig.widht=10, fig.height=5>>=
...
\begin{figure}[!hbt]
  \begin{center}
<<plotfmseq, dev ='pdf', fig.widht=10, fig.height=5>>=
...
\begin{figure}[!hbt]
  \begin{center}
<<plotfmid, dev ='pdf', fig.widht=10, fig.height=5>>=
plotFragmentMatching(synobj2,
...
The \Rfunction{plotFragmentMatchingPerformance} function can be used to
...
\begin{figure}[!hbt]
  \begin{center}
<<plotfmperformance, dev='pdf', fig.widht=8, fig.height=4>>=
...
columns that correspond to the fragment matching procedure,
e.g. \code{FragmentMatching}, \ldots.
