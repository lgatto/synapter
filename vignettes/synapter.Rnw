...
The rationale underlying \Rpackage{synapter}'s functionality are described in \cite{Shliaha2013} and \cite{Bond2013}.
The first reference describes the benefits of ion mobility separation on identification and the effects on quantitation, 
that led to the development of \Rpackage{synapter}, which in described and demonstrated in \cite{Bond2013}.
