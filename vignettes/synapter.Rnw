...
More details and screenshots are available in a separate document available at \url{http://lgatto.github.com/synapter/}.
...
Two functions are needed to choose a set of IR files and create the \emph{master} IR. One function enables to perform a complete identification transfer, eiter through a command line or graphical interface. Table \ref{tab:summary} summarises all there is to know to utilise \Rpackage{synapter}'s functionality.
