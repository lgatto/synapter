...
data(synobj3, package="synapterdata")
```
...
The example data `synobj3` used throughout this document is available in the
`r Biocpkg("synapterdata")` package and can be directly load as follows:

```{r loadsynobj3, eval=FALSE}
library("synapterdata")
data(synobj3)
```

In the [next section](\sec:synergise] we describe how `synobj3` was generated.
...
```
```{r create_synobj3}
synobj3 <- synergise1(object=synobj3,
                      outputdir=tempdir())
```
...
plotCumulativeNumberOfFragments(synobj3,
...
```{r fm}
setFragmentMatchingPpmTolerance(synobj3, 25)
...
m <- plotFragmentMatchingPerformance(synobj3)
```
...
oldEMRTtable <- getEMRTtable(synobj3)
performance(synobj3)
getEMRTtable(synobj3)
...
In this example we rescued `r getEMRTtable(synobj3)["1"]-oldEMRTtable["1"]` unique
...
```{r export, eval=FALSE}
writeMatchedEMRTs(synobj3, file = "MatchedEMRTs.csv")
```
