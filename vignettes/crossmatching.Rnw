...
\item identification spectra vs the quantitation fragments (results
  stored as \\ \code{spectrum.identXfragments.quant}),

\item the quantitation spectra vs the identification fragments
  (results stored as \\ \code{spectrum.quantXfragments.ident}), 

\item the identification vs the quantitation fragments
  (results stored as \\ \code{fragments.identXfragments.quant}).
...
plotCrossMatching(synobj2, key = 28,
                  column = "spectrum.quantXfragments.ident",
                  verbose = FALSE)
@
\caption{Fragment matching for cases with 28 common fragments. The
  identification (top, blue) and quantitation (bottom, red) low energy
  spectra are shown on the left. The high energy identification (top,
  blue) and quantitation (bottom, red) fragments are show on the
  right. Common peaks between the identification spectrum and
  quantitation fragments, quantitation spectrum and identification
  fragments and identification and quantitation fragments that are
  shared are displayed in darker colours and highlighted by full
  points on the identification spectrum, quantitation spectrum and
  fragment spectra respectively. }
    \label{fig:xm1}
  \end{center}
\end{figure}
...
\code{fragments.identXfragments.quant}, \ldots.  
