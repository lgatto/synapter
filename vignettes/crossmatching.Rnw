...
\begin{abstract}
  ...
  MS$^E$/HDMS$^E$ data using the \Rpackage{synapter}. The fragment matching
  feature allows you to rescue many non unique matches and to get rid of false
  assigned unique matches as well.
\end{abstract}
...
In this vignette we introduce the new fragment matching feature. This feature
improves the matching of the identification and the quantitation run. After
applying the usual \Rfunction{synergise} workflow you may end up with more or
less unique matches and a few false unique matches. The fragment matching
procedure allows you to remove false unique matches and increase the number of
unique true matches by resolving most of the non unique match situations.

\section{Running \Rfunction{synergise}}

You have to run the \Rfunction{synergize} workflow before you could
apply the fragment matching.
...
\section{Loading the spectrum  and fragment data}

The data needed for the fragment matching procedure are a \code{Spectrum.xml}
and a \code{final\_fragment.csv} file for the identification and the
quantitation run.
...
\section{Filtering fragments}

This is an optional step. If you have to many less abundant fragments in your
raw files you could filter them using \Rfunction{filterFragments}. To visualize
the intensity of all fragments you could use
\Rfunction{plotCumulativeNumberOfFragments}.
Both functions have a \code{what}
argument to decide which spectra/fragments you want to filter. Choose
\code{spectrum.ident} for the identification spectra, \code{spectrum.quant} for
the quantitation spectra, \code{fragments.ident} for the identification
fragments and \code{fragment.quant} for the quantitation fragments.
...
We named the method that is doing the fragment matching
\Rfunction{crossMatching} because we compare the identification spectra vs the
quantitation fragments \\
(\code{spectrum.identXfragments.quant}), the quantitation
spectra vs the identification fragments (\code{spectrum.quantXfragments.ident})
and the identification vs the quantitation fragments
(\code{fragments.identXfragments.quant}). Because the peaks/fragments in the
spectra of one run will never be numerical identical to these in another run you
have to specify a tolerance parameter via
\Rfunction{setCrossMatchingPpmTolerance}. Peaks/Fragments within this tolerance
...
There is the \Rfunction{plotCrossMatching} to show the details of this cross
matching procedure. If it is called without any additional argument every
cross matching is plotted. You can use the \code{key} argument to select a
special value in a column (given by the \code{column} argument) of the
\code{MatchedEMRTs} data.frame. E.g. we want to select the cross matching
results with a high number of common peaks, e.g. 22 common peaks:
<<plotcx22>>=
plotCrossMatching(synobj2, key = 22, column = "spectrum.quantXfragments.ident",
                  verbose = FALSE)
@

Or we may be interested in all results for the peptide with the sequence
\code{"TALIDGLAQR"}.
