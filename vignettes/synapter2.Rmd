...
There is also an `synobj3` object in `r Biocexptpkg("synapterdata")` which
...
```{r create-synobj3-file, results="asis", echo=FALSE, comment=NA}
cat(readLines(system.file(file.path("scripts", "create_synobj3.R"),
...
```{r filtering}
filterUniqueDbPeptides(synobj3,
                       missedCleavages=0,
                       IisL=TRUE)
filterPeptideLength(synobj3, l=7)
plotFdr(synobj3)
filterQuantPepScore(synobj3, method="BH",
                    fdr=0.05)
filterIdentPepScore(synobj3, method="BH",
                    fdr=0.05)
par(mfcol=c(1, 2))
plotPpmError(synobj3, what="Ident")
plotPpmError(synobj3, what="Quant")
par(mfcol=c(1, 1))
filterQuantPpmError(synobj3, ppm=20)
filterIdentPpmError(synobj3, ppm=20)
plotPepScores(synobj3)
filterIdentProtFpr(synobj3, fpr=0.05)
filterQuantProtFpr(synobj3, fpr=0.05)
```
...
```{r rtmodel}
mergePeptides(synobj3)

plotRt(synobj3, what="data")
setLowessSpan(synobj3, span=0.05)
modelRt(synobj3)

par(mfcol=c(1, 2))
plotRtDiffs(synobj3)
plotRt(synobj3, what="model", nsd=1)
par(mfcol=c(1, 1))

plotFeatures(synobj3, what="all", ionmobility=TRUE)
```
...
setBestGridParams(synobj3)
findEMRTs(synobj3)
plotEMRTtable(synobj3)
```
...
```{r fm}
fragmentMatching(synobj3)
```

We get tables for unique and non-unique matches:

```{r fmperf, results="asis"}
knitr::kable(fragmentMatchingPerformance(synobj3,
                                         what="unique"))
knitr::kable(fragmentMatchingPerformance(synobj3,
                                         what="non-unique"))
```
...
```{r filterfm}
filterUniqueMatches(synobj3, minNumber=1)
filterNonUniqueMatches(synobj3, minDelta=2)
filterNonUniqueIdentMatches(synobj3)
```

Finally we rescue EMRTs that are filtered but were identified by *PLGS*:

```{r rescue}
rescueEMRTs(synobj3, method="rescue")
```
...
```{r intmodel}
plotIntensity(synobj3, what="data")
setLowessSpan(synobj3, 0.05)
modelIntensity(synobj3)
plotIntensity(synobj3, what="model", nsd=1)
```
...
synobj3 <- synergise2(filenames = inlist,
                      outputdir = ".")
```
