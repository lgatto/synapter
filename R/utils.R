...
# split isotope distribution information
# @param x vector of isotopicDistr strings, generated by .catIsotopeDistr
# @return a list of isotopic counts named with their isotopic name
# @noRd
.splitIsotopicDistr <- function(x) {
  nm <- names(x)
  x <- strsplit(x, ":|;")
  n <- length(x)
  ...
  nall <- sum(ne)
  x <- unlist(x)
  x <- setNames(as.numeric(x[seq(from=2, to=nall, by=2)]),
                x[seq(from=1L, to=nall, by=2L)])
