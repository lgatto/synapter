...
common <- c(NA, NA)
selCommon <- tail(which(colSums(t(fragmentMatchingStats[[1]][-1, c("ncommon", "fdr")]) <= c(fm.minCommon, fm.fdr.unique)) == 2), 1)

if (length(selCommon)) {
  common <- fragmentMatchingStats[[1]][-1, c("ncommon", "fdr")][selCommon,]
  filterUniqueMatches(obj, minNumber = common[1L])
}

delta <- c(NA, NA)
selDelta <- tail(which(colSums(t(fragmentMatchingStats[[2]][-1, c("deltacommon", "fdr")]) <= c(fm.minDelta, fm.fdr.nonunique)) == 2), 1)

if (length(selDelta)) {
  delta <- fragmentMatchingStats[[2]][-1, c("deltacommon", "fdr")][selDelta,]
