...
test_that("requantifySum, only common isotopes", {
  ...
  expect_equal(synapter:::.requantifySum(f, Inf), int)
  expect_equal(synapter:::.requantifySum(f, 5000), sat5000_int)
})
...
test_that("requantifySum, all isotopes below threshold", {
  ...
  expect_equal(synapter:::.requantifySum(f, Inf,
                                         keepLonelyIsotopes = TRUE),
               int)
  expect_equal(synapter:::.requantifySum(f, 5000,
                                         keepLonelyIsotopes = TRUE),
               sat5000_int)
})
